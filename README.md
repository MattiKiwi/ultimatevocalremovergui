...
### Headless / CLI usage with uv

1. Install the dependencies into an isolated environment (managed via `pyproject.toml`):

   ```bash
   bash scripts/setup_uv.sh
   ```

2. Download the ONNX/PyTorch models you want to run and place them in the appropriate subfolders under `models/`. The manual download list lives in `gui_data/model_manual_download.json`.

3. Separate a track using the lightweight CLI (defaults to `UVR-MDX-NET-Inst_HQ_3.onnx` if present). Missing models are fetched automatically from the public UVR mirror:

   ```bash
   uv run --python .venv/bin/python -m uvr_cli path/to/song.wav \
     --model models/MDX_Net_Models/UVR-MDX-NET-Inst_HQ_3.onnx
   ```

   Results are written to `UVR_separated/` (configurable with `--output-dir`).

4. You can also call the separator from your own scripts:

   ```python
   from uvr_cli import separate

   result = separate(
       "path/to/song.wav",
       model_path="models/MDX_Net_Models/UVR-MDX-NET-Inst_HQ_3.onnx",
       output_dir="my_stems",
   )
   print(result.primary, result.secondary)
   ```
